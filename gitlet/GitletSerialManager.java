 ...
 * @author Winston
 */
public class GitletSerialManager extends Serializer<GitletSerializer> {
    /**
     * dir delimiter.
     */
    private static final int DIR_DEL = 2;

    /**
     * constructor.
     * @param base the base path
     */
    public GitletSerialManager(Path base) {
        super(base);
    }

    @Override
    public <S extends GitletSerializer> S get(Class<S> type, String fileName) {
        return super.get(type, hashToFile(fileName));
    }
     ...
     */
    @Override
    public <S extends GitletSerializer> void forEach(
            Class<S> type,
    ...
    @Override
    public <S extends GitletSerializer> boolean contains(Class<?> type,
                                                         String hash) {
     ...
     */
    public String put(GitletSerializer obj) {
     ...
     * @param objs objects
     */
    public void putAll(Collection<? extends GitletSerializer> objs) {
        for (GitletSerializer q : objs) {
            this.put(q);
        }
    }
     ...
     */
    @Override
    public <S extends GitletSerializer> void remove(
            Class<S> type, String hash) {
    ...
    private static String hashToFile(String hash) {
        return hash.substring(0, DIR_DEL) + "/"
                + hash.substring(DIR_DEL, hash.length());
    }
     ...
     * @param search delimiter with which to search.
     * @param <S> s
     * @return The gitlet object find.
     */
    public <S extends GitletSerializer> S find(Class<S> type, String search) {
        ...
        String delim =
                search.substring(0, Math.min(search.length(), DIR_DEL));

        String rest = "";
        if (search.length() > DIR_DEL) {
            rest = search.substring(DIR_DEL, search.length());
        }
